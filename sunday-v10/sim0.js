...
function prep(t,p,area){if(t.code!==S.state.coach.team||!p)return 0;let b=(p.practiceReps-50)/60;if(p.practiceFocus==='Run Blocking'&&area==='run')b+=.75;if(p.practiceFocus==='Pass Protection'&&area==='pass')b+=.75;if(p.practiceFocus==='Route Detail'&&area==='route')b+=.55;if(p.practiceFocus==='Hands'&&area==='hands')b+=.45;return b}
...
function runBlock(G,t,o,r){const line=ol(t);if(!line.length)return 0;const p=choice(r,line),x=gs(G,p),skill=((p.attrs.runBlock||p.overall)-74)/14+prep(t,p,'run')+(t.code===S.state.coach.team?(S.state.morale.olChemistry-50)/18:0)-(o.power-t.power)/40;const win=r()<S.clamp(.53+skill*.07,.28,.77);win?x.runBlockWins++:x.runBlockLosses++;return win?normal(r,.65,.6):-Math.max(.2,normal(r,1,.7))}
