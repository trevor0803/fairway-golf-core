...
function weighted(r,a,w){let total=a.reduce((n,x)=>n+Math.max(0,w(x)),0),z=r()*total;if(total<=0)return a[0];for(const x of a){z-=Math.max(0,w(x));if(z<=0)return x}return a[a.length-1]}
...
function depthRank(p){return Number.isFinite(+p.depthRank)?+p.depthRank:99}
function rotation(p,kind='snap'){
 if(Number.isFinite(+p.rotationShare))return Math.max(.03,Math.min(.99,+p.rotationShare));
 const d=depthRank(p),pos=p.pos;
 if(pos==='QB')return d===1?.995:d===2?.025:.005;
 if(pos==='RB')return d===1?.58:d===2?.29:d===3?.11:.035;
 if(pos==='WR')return d===1?.9:d===2?.82:d===3?.68:d===4?.34:d===5?.16:.07;
 if(pos==='TE')return d===1?.76:d===2?.4:d===3?.16:.06;
 if(['OT','OG','C','OL'].includes(pos))return d<=1?.98:d===2?.2:.05;
 if(pos==='DL')return d===1?.68:d===2?.5:d===3?.3:.14;
 if(pos==='LB')return d===1?.78:d===2?.48:d===3?.22:.1;
...
function pickRB(t,r,G){const a=backs(t).filter(p=>active(G,p));return weighted(r,a,p=>rotation(p,'carry')*(1+(p.overall-72)/60)*(G.rbBias[p.id]||1)*(1+prep(t,p,'run')*.025))}
function pickTarget(t,r,kind,G){const a=elig(t).filter(p=>active(G,p));if(!a.length)return null;const plan=gp(t,G);return weighted(r,a,p=>{let w=p.pos==='WR'?1:p.pos==='TE'?.7:.25;if(kind==='screen')w=p.pos==='RB'?2.7:p.pos==='WR'?.4:.14;else if(kind==='check')w=p.pos==='RB'?1.6:p.pos==='TE'?.9:.38;if(plan.rbPassGame&&p.pos==='RB')w*=1.08;w*=Math.max(.06,rotation(p,'target'));w*=1+(p.overall-72)/90;w*=G.targetBias[p.id]||1;w*=1-S.clamp((p.fatigue-38)/110,0,.25);w*=1+prep(t,p,'route')*.035;return w})}
function tackle(G,t,r,kind){const a=defense(t);if(!a.length)return;const p=weighted(r,a,x=>rotation(x,'def')*(kind==='run'?(x.pos==='LB'?1.5:x.pos==='DL'?1.2:.7):(x.pos==='DB'?1.5:x.pos==='LB'?1:.4)));gs(G,p).tackles++}
function defSack(G,t,r){const a=defense(t);if(a.length)gs(G,weighted(r,a,x=>rotation(x,'def')*(x.pos==='DL'?1.8:x.pos==='LB'?1:.15))).defSacks++}
function defInt(G,t,r){const a=defense(t);if(a.length)gs(G,weighted(r,a,x=>rotation(x,'def')*(x.pos==='DB'?1.9:x.pos==='LB'?.7:.08))).defInt++}
function addSnaps(G,t,pass,r){for(const p of ol(t)){const x=gs(G,p);x.snaps++;pass?x.passBlockSnaps++:x.runBlockSnaps++}for(const p of elig(t)){if(!active(G,p))continue;const share=rotation(p,'snap');if(r()<Math.min(.99,Math.max(.03,share))){const x=gs(G,p);x.snaps++;if(pass&&p.pos!=='RB')x.routes++}}}
